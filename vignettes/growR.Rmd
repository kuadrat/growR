...
In short, ModVege takes a values relating to daily weather conditions, 
...
```{r, include = FALSE}
# knitr resets working dir after every chunk.
# Store the original wd and permanently move to working_dir.
old_dir = getwd()
knitr::opts_knit$set(
...
```{r run_simulation}
results = growR_run_loop(environments, store_results = TRUE)
```
...
```{r ni_screening}
new_results = growR_run_loop(new_envs, write_files = FALSE, 
                             store_results = TRUE)
# Plot all results
...
```{r, include = FALSE}
# Clean up is not necessary because knitr deletes its own tmp dir when done.
# Clean up
#knitr::opts_knit$set(
#  root.dir = old_dir
#)
